# Looker Explore Assistant - Claude Instructions
...
```
**Important**: Use `python3` vs `python` for local development.

### Testing & Quality Assurance
- Always run tests after making changes: `npm test` or `pytest`
- Check linting with `npm run lint` (frontend) or `flake8` (backend)
...
## Key Files That Define Core Patterns
...
- Check `package.json` or requirements files before adding new dependencies
- Use TypeScript for frontend, Python for backend
- Follow Redux Toolkit patterns for state management
- Use hooks-based React patterns

### Testing & Quality
- Run `npm run lint` and `npm run typecheck` after frontend changes
- Run `flake8` or equivalent linting for Python changes
- Test changes locally using the development commands above
- Verify OAuth flows work end-to-end when touching authentication

### Security Considerations
- Never expose API keys or credentials in client-side code
- Use environment variables for sensitive configuration
- Validate all user inputs, especially for BigQuery operations
- Follow OAuth2 best practices for token handling